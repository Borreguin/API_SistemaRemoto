...
def get_values_for_tags(tag_list: TagListRequest):
    ...
    for tag in tag_list.tags:
        ...
        if point.pt is None:
            result.append(dict(name=tag.tag_name, value=None, timestamp=None, id=str(tag).lower()))
            continue
        snapshot = point.snapshot()
        if isinstance(snapshot, dict):
            snapshot['id']= str(tag.tag_name).lower()
            result.append(snapshot)
            continue
        value, timestamp = str(snapshot.Value), snapshot.Timestamp.LocalTime
        result.append(dict(name=point.pt.Name, value=value, timestamp=str(timestamp), id=str(tag.tag_name).lower()))
    return result
