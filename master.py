...
def processing_master(_file_path: str, ini_date: dt.datetime, end_date: dt.datetime):
    ...
    for node in nodes:
        ...
        with io.open(log_path, mode='wb') as out:
            p = sb.Popen(
                ["python", "node.py", node, ini_date.strftime(yyyy_mm_dd), end_date.strftime(yyyy_mm_dd), "--s"],
...
if __name__ == "__main__":
    ...
    parser.add_argument("archivo", help="indica el path del archivo master a leer (masters path)",
                        type=str)
